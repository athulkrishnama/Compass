...
@injectable()
export class GetTravelerCompletedBookingsUseCase
  implements IGetTravelerCompletedBookingsUseCase
{
  constructor(
  ...
  async execute(
    ...
    const data = await this._hotelBookingRepo.getTravelerCompletedBookings(
      travelerId,
      pageNo,
    );
