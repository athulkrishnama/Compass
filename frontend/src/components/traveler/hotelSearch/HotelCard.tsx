...
export const HotelCard = ({ hotel }: HotelCardProps) => {
    ...
    const [showRoomVariants, setShowRoomVariants] = useState(false);

    const hasRoomVariants = hotel.roomVariants && hotel.roomVariants.length > 0;
    const hasMultipleRoomVariants =
        hotel.roomVariants && hotel.roomVariants.length > 1;
        ...
        >
            <div className="relative aspect-[4/3] overflow-hidden flex-shrink-0">
                ...
                {hasRoomVariants && (
                    <Badge className="absolute top-3 right-3 bg-black/80 text-white">
                        ...
                        {t(translationKey.hotelSearch.availableRoomVariants)}
                    </Badge>
                )}
            </div>
            ...
            <div className="p-4 flex flex-col flex-grow">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-zinc-100 mt-auto">
                    ...
                    {hasMultipleRoomVariants && (
                        <Button
                            variant="ghost"
                            size="sm"
                            className="text-xs text-zinc-600 hover:text-black"
                            onClick={() =>
                                setShowRoomVariants(!showRoomVariants)
                            }
                        >
                            {showRoomVariants ? "Hide" : "Show"} rooms
                        </Button>
                    )}
                </div>

                <AnimatePresence>
                    {showRoomVariants && hasRoomVariants && (
                        <motion.div
                            initial={{ height: 0, opacity: 0 }}
                            animate={{ height: "auto", opacity: 1 }}
                            exit={{ height: 0, opacity: 0 }}
                            className="mt-3 pt-3 border-t border-zinc-100 overflow-hidden"
                        >
                            <div className="space-y-2">
                                {hotel.roomVariants.map(
                                    (variant: RoomVariant, index: number) => (
                                        <div
                                            key={variant._id || index}
                                            className="flex items-center justify-between p-2 bg-zinc-50 rounded-lg"
                                        >
                                            <div>
